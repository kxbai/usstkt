...
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC,ENABLE);	//打开GPIO口时钟，先打开复用才能修改复用功能

	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);	         //SW1, SW3, SW2
	GPIO_InitStructure.GPIO_Pin =GPIO_Pin_0; //PB14 PB5  PB10	 
 	GPIO_InitStructure.GPIO_Mode =GPIO_Mode_IPU; 		 //内部上拉输入
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;//速度50MHz
 	GPIO_Init(GPIOB, &GPIO_InitStructure);

//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);	//打开GPIO口时钟，先打开复用才能修改复用功能

	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);	         //SW1, SW3, SW2
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_0; //PB14 PB5  PB10	 
 	GPIO_InitStructure.GPIO_Mode =GPIO_Mode_IPU; 		 //内部上拉输入
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;//速度50MHz
...
{
  // Get button pressed states                 ---Song---
