...
//  GPIO_PinRemapConfig(GPIO_PartialRemap_TIM1, ENABLE);       //TIM2 Remap CH1->PA15 
   ...
   //设置该引脚为复用输出功能,输出TIM2 CH1的PWM脉冲波形
	...
	GPIO_ResetBits(GPIOA,GPIO_Pin_11);  //蜂鸣器对应引脚GPIOF8拉低，
...
//	ocr = icr - (icr / (32>>vol));
	...
	TIM_SetAutoreload(TIM1,ocr);  //改变频率
