...
static void dso(unsigned char data,unsigned char delta)
{
	...
	for(cnt=0;  cnt <=127/delta ;cnt++)//屏幕宽度0-127
	{
		...
		if(showcnt>=256)
		{
			showcnt=256-point_delta+showcnt;//回到数组尾部
		}
	}
	//把点连成线
...
{
	char buf[12];
	sprintf(buf,"Guan:%.6d",Pressure_Dis_Guan);
...
//	dso(roll,delta);
  dso(Pressure_Dis_Guan/3000,delta);
//  dso(50,delta);
	return DISPLAY_BUSY;
}

static display_t draw1()
{
	char buf[12];
	sprintf(buf,"Pulse Send");
	draw_string((char*)buf, false, 0, 0);
	return DISPLAY_BUSY;
}
